...
class TB_Map_Module extends Themify_Builder_Component_Module {
	...
	<# 
		...
		<#
		    ...
		    args.reverse = !address && args.latlong_map;
		    args.address = address || args.latlong_map,
		    args.scroll = args.scrollwheel_map == 'enable'?1:0,
		    args.drag = args.draggable_map == 'enable'?1:0,
		    args.mdrag=args.draggable_disable_mobile_map=='yes'?1:0,
		    args.control = args.map_control == 'no' ? 1 : 0,
		    args.type= args.map_provider == 'google'?args.type_map:args.bing_type_map;
		#>
		    <div data-map-provider="{{ args.map_provider }}" data-address="{{ args.address }}" data-control="{{args.control}}" data-zoom="{{ args.zoom_map }}" data-type="{{ args.type }}" data-scroll="{{ args.scroll }}" data-drag="{{ args.drag }}" data-mdrag="{{ args.mdrag }}" class="themify_map<# print(args.map_provider != 'google'?' themify_bing_map':'')#>"  style="{{ style }}"  data-info-window="{{ args.info }}" data-reverse-geocoding="{{ args.reverse }}"></div>
		<# } #>
