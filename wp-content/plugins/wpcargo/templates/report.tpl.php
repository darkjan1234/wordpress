<form id="wpc-ie-form" method="POST" action="<?php echo admin_url(); ?>edit.php?post_type=wpcargo_shipment&page=wpc-report-export" >
			...
			<p id="daterange-section" style="padding-left: 20px;">
				...
				<input class="wpcargo-datepicker" type="text" id="wpcargo-import-form" name="date-from" value="<?php echo isset($_REQUEST['date-from']) ? esc_html( $_REQUEST['date-from'] ) : ''; ?>" required />
				<label for="date-to"><strong><?php esc_html_e('To : ','wpcargo'); ?></strong></label>
				<input class="wpcargo-datepicker" type="text" id="wpcargo-import-to" name="date-to" value="<?php echo isset($_REQUEST['date-to']) ? esc_html( $_REQUEST['date-to'] ) : ''; ?>" required />
			</p>
			<p>
			...
			<select name="wpcargo_status" class="wpc-status">
				...
				if(!empty($wpcargo->status)) {
					...
					foreach( $wpcargo->status as $status ){
						$get_selected_value = isset($_REQUEST['wpcargo_status']) && esc_html( $_REQUEST['wpcargo_status'] ) == $status ? 'selected' : '';
						echo '<option value="'.esc_html( $status ).'" '.esc_html($get_selected_value).'>'.esc_html($status).'</option>';
					}
				}
				?>
			</select>
                        ...
                        <select name="from[]" id="multiselect" class="form-control" size="8" multiple="multiple">
                            ...
                            if($fields) {
                                ...
                                foreach( $fields as $key => $value ){
                                    ?><option value="<?php echo esc_html($value['meta_key']); ?>"><?php echo esc_html($value['label']); ?></option><?php
                                }
                            }
                            ?>
                        </select>
                            ...
                            <?php
                                if(!empty( $options ) ) {
                                    foreach ($options as $optkey => $optvalue ) {
                                    	echo "<option value='".esc_html($optkey)."'>".esc_html($optvalue)."</option>";
                                    }
                                }
                            ?>
                        </select>
